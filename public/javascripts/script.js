...
socket.on('connect', () => {
  console.log(`id from frontend... ${socket.id}`)
})

socket.on('startGame', (id) => {
  console.log(`server responded with id: ${id}`)
  isRefree = socket.id===id;
  startGame();
});

socket.on('paddleMoved', (data) => {
  const paddleIndexP = 1 - paddleIndex
  paddleX[paddleIndexP] = data.xPos;
})
